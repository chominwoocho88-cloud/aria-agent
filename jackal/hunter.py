...
def _stage2_orca_context(top50: list, aria: dict) -> list:
    """
...
def _load_compact_ctx() -> str:
    """
    jackal.compact가 저장한 압축 컨텍스트 로드.
...
def _stage4_full_analysis(top10: list, aria: dict) -> list:
    ...
    for item in top10:
        ...
        raw_signal_family = analyst.get("swing_type", "")
        signal_family = canonical_family_key(
            swing_type=raw_signal_family,
...
def run_hunt(force: bool = False) -> dict:
    ...
    try:
        ...
        atomic_write_json(_wf, _w)
        record_jackal_weight_snapshot(_w, source="hunter_macro_gate")
    except Exception:
        pass
    ...
    # ── Stage 2: 50 → 25 ─────────────────────────────────────────
    top25 = _stage2_orca_context(top50, aria)
